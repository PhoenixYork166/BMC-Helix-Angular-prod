...
/**
 *
 * @type {{BACKSPACE: number, TAB: number, RETURN: number, ESC: number, SPACE: number, PAGE_UP: number, PAGE_DOWN: number, END: number, HOME: number, LEFT: number, UP: number, RIGHT: number, DOWN: number, DELETE: number}}
 */
aria.KeyCode = {
  BACKSPACE: 8,
  TAB: 9,
...
aria.Utils.matches = function (element, selector) {
  if (!Element.prototype.matches) {
    Element.prototype.matches =
      ...
      function (s) {
        let matches = element.parentNode.querySelectorAll(s);
 ...
 */
aria.Grid = function (gridNode, msg) {
...
/**
 * @desc
 *  Adds a message to the helper left bottom corner
 */

aria.Grid.prototype.addMessage = function (msg) {
  var m = document.createTextNode(msg || 'activated');
  this.keysIndicator.appendChild(m);
};
...
aria.Grid.prototype.setupFocusGrid = function () {
  ...
  Array.prototype.forEach.call(
    ...
    (function (row) {
      ...
      Array.prototype.forEach.call(row.querySelectorAll(aria.GridSelector.CELL),
        (function (cell) {
          let focusableSelector = '[tabindex]';

          if (aria.Utils.matches(cell, focusableSelector)) {
            rowCells.push(cell);
          }
          else {
            let focusableCell = cell.querySelector(focusableSelector);
...
aria.Grid.prototype.isHidden = function (row, col) {
  let cell = this.gridNode.querySelectorAll(aria.GridSelector.ROW)[row]
